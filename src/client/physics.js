...
class Physics {
  ...
  update() {
    ...
    let mup = this.up && !this.down;
    let mdown = !this.up && !this.down;
    let fh = this.fh;
    let delta = 0.01;
    if (this.lr) {
    } else if (this.fh) {
      ...
      let fx2 = fx * fx;
      let fy2 = fy * fy;
      let len = Math.sqrt(fx2 + fy2);
      let mvr = (this.vx * len) / fx;
      mvr -= fh.force;
      // info.fs
    ...
    while (delta > epsilon) {
      ...
      if (fh) {
        ...
        let nnx = x + dx1;
        let nny = y + dy1;
        for (let f of MapleMap.footholds || []) {
          let dx2 = f.x2 - f.x1,
            dy2 = f.y2 - f.y1;
          let dx3 = x - f.x1,
            dy3 = y - f.y1;
